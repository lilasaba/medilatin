...
% TSD 2017: For more than 2 authors, please, use the following
 \authorrunning{Szab\'{o}, Lili et al.}
...
\and
THINKTech Research Center \\ % már a saját cégem nevét sem tudom, bocs :~
\mailsb\\
% \url{www.website.org} \\
\and
...
\begin{table}
	...
	\hline
	Orthography & \v{r}ekl & \H{o}z & mi\'{s} & ap\u{a} \\  % nem vagyok biztos benne, de nekem így természetesebb...
	USG & rekl & oz & mis & apa \\
	\hline
	\end{tabular}
\end{table}
...
It is worth mentioning that the Czech and Slovak speaker test sets achieve a surprisingly low $6.4\%$ and $9.1\%$ WER respectively on the Hungarian text test set. % én csak ezeket boldítanám be.
...
\end{table}
The most striking result was obtained by the comparison of Table~\ref{tbl:hu_phoneme} and Table~\ref{tbl:cz_hu_pl_ro_usg} Hungarian speakers' results on Hungarian test text.
% az alábbi nem igaz, mert a lengyel is jobb az USG-vel a magyaron. 
%all but the Hungarian speaker test sets performed better on the Hungarian text test set.


We had expected the opposite results, i.e., the Hungarian speakers to perform better with the Hungarian knowledge-based model and Hungarian text test set setting, but in fact the phoneme mapping masked the difference between mid-front \texttt{/e:/} and open-front \texttt{/\textipa{E}/} in the pronunciation of the Hungarian speakers.
In addition to that, they were pronouncing the named entities using their native pronunciation, which also increased the WER in the case of the phonetic approach.
