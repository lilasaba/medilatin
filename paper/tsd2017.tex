...
The results show that the experiments conducted on the Hungarian origin text test set yielded to the best results with all models.
...
The results for the Czech-Hungarian-Polish joint acoustic model are in Table~\ref{tbl:three_lang_results}, along with the other three-language USG model results.
For the four-language USG model in Table~\ref{tbl:three_lang_results} we used the smaller (112 hours) Hungarian training data to avoid over-weighting.
This setting yielded to a significantly better overall $20.4\%$ WER. 
As Table~\ref{tbl:three_lang_results} shows, each language contributed almost evenly to the four-language USG model.
To avoid overweighting, in all these settings, the Hungarian data set was reduced to 112 hours of Broadcast News speech.  
%pool of recognizers
\begin{table}
\parbox{.45\linewidth}{
\centering
\caption{WER[\%] of USG model of Czech, Hungarian and Polish (CZ+HU+PL).}
\input{tables/cz_hushort_pl_ro_usg.tex}\label{tbl:three_lang_results}
}
\hfill
\parbox{.45\linewidth}{
\centering
