...
% még nem érzem elég találónak a címet. Ez igazábó nem nyelvfüggetlen modellezés, csak mentes az in-domain adattól. A "target" szó sem illik a címbe... De ez még jobban ráér...
...
The acoustic models we start with are monolingual grapheme- and/or phoneme-based ones (depending of the language resources available) for these three languages. 
In one hand, the application of medieval Latin knowledge based grapheme-to-phoneme (G2P) mapping from the source language to the target language resulted in significant improvement (...). 
On the other hand, applying a simplified, unified grapheme inventory set for the three-language acoustic data set complemented with Romanian speech data yielded in competitive results - without using any target or source language G2P rules.
...
It is worth mentioning that the Czech speakers achieve a surprisingly low $7.3\%$ WER on the Hungarian text test set.
...
We also tried adding a new source language, Romanian, to the joint USG acoustic model, which improved the results significantly, yielding in an overall $29.3\%$ WER, with a best WER if $12.1\%$ of the Czech speakers on the Hungarian test test, see in Table~\ref{tbl:cz_hu_pl_ro_usg}.
...
\parbox{.45\linewidth}{
\centering
\caption{USG model of Czech (76 hours), Hungarian (112 hours), Polish (31 hours) and Romanian (35 hours).}
