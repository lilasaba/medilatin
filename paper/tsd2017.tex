...
\title{Language Specific Knowledge and Data Free Acoustic Modeling for Medieval Latin LVCSR}
...
Such language technology tool can be of great help for preserving Latin language charters from this era, as optical character recognition systems are often challenged by these historic documents.
...
The baseline acoustic models we start with are monolingual grapheme-based ones -  according to the language resources available - for these three languages. 
In one hand, the application of medieval Latin knowledge-based grapheme-to-phoneme (G2P) mapping from the source language to the target language resulted in significant improvement, reducing the WER by $17.3\%$. 
...
% A következő már nem kell, mert betettük visszatettük a szlovákot a cikkbe. Annak ellenére sem kell, hogy tanítóadat nincs rá, de nem is érdekes, csak használni akarják és teszt van.
%\footnote{Sadly, but for this paper, although it belongs to the Visegrad region, the Slovakian language is not included in the experiments, due to the lack of speech data for acoustic model training.}
%The system we develop is a joint system that can deal with both the variability in the speakers' pronunciations when speaking medieval Latin, as well as the grammatical/lexical variabilities of the target texts. %ebben nem találtam új infót az előzőekhez képest.
...
%TODO: \cite{schultz01} ez is legyen
%TODO: \cite{bryne00}
%TODO: \cite{besacier14} meg ez is!
...
% szerintem ezt az alábbi 2 részt egybe kellene venni. Hiszen a két lépés inkább egyszerre, vagy legalábbis fordított sorrendben történik. Amúgy nem egymáshoz hasonlítjuk a nyelvek grafémáit, hanem a latinhoz és ami eltér, azt egyszerűsítjük.
...
\input{tables/cz_hu_pl_grapheme.tex}\label{tbl:cz_hu_pl_grapheme}
\end{table}
