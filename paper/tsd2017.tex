...
The baseline acoustic models we start with are monolingual grapheme-based ones for these three languages - according to the language resources available. 
...
Our baseline system consists of separately trained grapheme-based acoustic models for three of the Visegrad languages (Czech, Hungarian, Polish) and Romanian is added as well.
...
% three-tree?? Nagyzolunk, vagy rosszul értem?
...
Native speakers of Czechia, Hungary, Poland and Slovakia - all of whom have experince with medieval Latin - were asked to record the three dev and test sets described in Section~\ref{testtext}.
...
The Hungarian knowledge-based acoustic model results are the second best ones, with an $21.1\%$ overall WER.
...
We also tried adding a new source language, Romanian, to the joint USG acoustic model, which improved the results significantly, yielding to our best experimental result of an overall $20.4\%$ WER, with a WER of $11.8\%$ of the Czech speakers on the Hungarian test test, see in Table~\ref{tbl:cz_hu_pl_ro_usg}.
